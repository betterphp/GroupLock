...
public class BlockLockableRedstoneRepeaterOff extends BlockDiode {
	
	public BlockLockableRedstoneRepeaterOff(){
		super(LockableBlock.REDSTONE_REPEATER_OFF.getType().getId(), false);
	...
	public void postPlace(World world, int x, int y, int z, EntityLiving entity){
		...
		Bukkit.getPluginManager().callEvent(new LockablePlacedEvent(block, player, lockable, LockableBlock.REDSTONE_REPEATER_OFF));
	}
	...
	public boolean interact(World world, int x, int y, int z, EntityHuman entity, int i1, float f1, float f2, float f3){
		...
		LockableOpenEvent event = new LockableOpenEvent(block, player, lockable, LockableBlock.REDSTONE_REPEATER_OFF);
