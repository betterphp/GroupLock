...
public class BlockLockableStonePressurePlate extends BlockPressurePlate {
	...
	public BlockLockableStonePressurePlate(){
		super(LockableBlock.STONE_PRESSURE_PLATE.getType().getId(), Block.STONE.textureId, EnumMobType.MOBS, Material.STONE);
	...
	public void postPlace(World world, int x, int y, int z, EntityLiving entity){
		...
		Bukkit.getPluginManager().callEvent(new LockablePlacedEvent(block, player, lockable, LockableBlock.STONE_PRESSURE_PLATE));
	}
	...
	public void onPlace(World world, int x, int y, int z){
		...
		world.setTileEntity(x, y, z, new TileEntityLockableStonePressurePlate());
	}
	...
	public boolean interact(World world, int x, int y, int z, EntityHuman entity, int i1, float f1, float f2, float f3){
		...
		LockableOpenEvent event = new LockableOpenEvent(block, player, lockable, LockableBlock.STONE_PRESSURE_PLATE);
