...
public class BlockLockableWoodButton extends BlockButton {
	
	public BlockLockableWoodButton(){
		super(LockableBlock.WOOD_BUTTON.getType().getId(), Block.WOOD.textureId, true);
	...
	public void postPlace(World world, int x, int y, int z, EntityLiving entity){
		...
		Bukkit.getPluginManager().callEvent(new LockablePlacedEvent(block, player, lockable, LockableBlock.WOOD_BUTTON));
	}
	...
	public void onPlace(World world, int x, int y, int z){
		...
		world.setTileEntity(x, y, z, new TileEntityLockableWoodButton());
	}
	...
	public boolean interact(World world, int x, int y, int z, EntityHuman entity, int i1, float f1, float f2, float f3){
		...
		LockableOpenEvent event = new LockableOpenEvent(block, player, lockable, LockableBlock.WOOD_BUTTON);
