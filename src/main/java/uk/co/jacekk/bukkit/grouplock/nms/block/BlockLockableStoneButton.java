...
public class BlockLockableStoneButton extends BlockButton {
	
	public BlockLockableStoneButton(){
		super(LockableBlock.STONE_BUTTON.getType().getId(), Block.STONE.textureId, false);
	...
	public void postPlace(World world, int x, int y, int z, EntityLiving entity){
		...
		Bukkit.getPluginManager().callEvent(new LockablePlacedEvent(block, player, lockable, LockableBlock.STONE_BUTTON));
	}
	...
	public void onPlace(World world, int x, int y, int z){
		...
		world.setTileEntity(x, y, z, new TileEntityLockableStoneButton());
	}
	...
	public boolean interact(World world, int x, int y, int z, EntityHuman entity, int i1, float f1, float f2, float f3){
		...
		LockableOpenEvent event = new LockableOpenEvent(block, player, lockable, LockableBlock.STONE_BUTTON);
